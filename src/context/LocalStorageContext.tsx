...
const initialFlashcards: Flashcard[] = [
  ...
  // AI flashcards
  {
    ...
    answer: 'Maskininlärning är en gren av artificiell intelligens där datorer "lär sig" från data och förbättrar sina algoritmer över tid utan att vara explicit programmerade för varje uppgift.',
    category: 'coding',
    difficulty: 'advanced',
  },
  {
    id: 'ai2',
    question: 'Vad är skillnaden mellan AI, maskininlärning och djupinlärning?',
    answer: 'AI (artificiell intelligens) är det breda konceptet att maskiner kan utföra uppgifter "smart". Maskininlärning är en delmängd av AI där maskiner lär sig från data. Djupinlärning är en delmängd av maskininlärning som använder neurala nätverk med många lager.',
    category: 'coding',
    difficulty: 'advanced',
  },
  {
    ...
    answer: 'Ett neuralt nätverk är en maskininlärningsmodell inspirerad av hjärnans struktur. Det består av lager av noder (neuroner) som processar data, identifierar mönster och gör beslut eller prognoser.',
    category: 'coding',
    difficulty: 'advanced',
  },
  {
    id: 'ai4',
    question: 'Vad är "övervakad inlärning"?',
    answer: 'Övervakad inlärning är en typ av maskininlärning där algoritmen tränas på märkt data. Den lär sig mappningen från input till output baserat på exempel-par, för att sedan kunna förutsäga outputs för nya inputs.',
    category: 'coding',
    difficulty: 'advanced',
  },
  {
    id: 'ai5',
    question: 'Vad är "oövervakad inlärning"?',
    answer: 'Oövervakad inlärning är en typ av maskininlärning där algoritmen tränas på omärkt data. Den söker efter mönster, grupperingar eller avvikelser i data utan förutbestämda kategorier eller resultat.',
    category: 'coding',
    difficulty: 'advanced',
  },
  ...
  // Geography flashcards
  {
    ...
    answer: 'En kontinent är en stor, sammanhängande landmassa. De sju kontinenterna är: Asien, Afrika, Nordamerika, Sydamerika, Europa, Australien/Oceanien och Antarktis.',
    category: 'geography',
    difficulty: 'beginner',
  },
  {
    id: 'geo2',
    question: 'Vad är en halvö?',
    answer: 'En halvö är en landmassa som sticker ut i en vattenmassa och är omgiven av vatten på tre sidor men ansluten till en större landmassa på en sida.',
    category: 'geography',
    difficulty: 'beginner',
  },
  {
    id: 'geo3',
    question: 'Vad är ett delta?',
    answer: 'Ett delta är en triangulär landformation som skapas av sedimentavlagringar vid en flodens mynning där den flyter ut i en sjö, ett hav eller en ocean.',
    category: 'geography',
    difficulty: 'beginner',
  },
  {
    ...
    answer: 'Väder avser de kortvariga atmosfäriska förhållandena (temperatur, nederbörd, vind) vid en specifik plats och tid. Klimat avser de genomsnittliga väderförhållandena i en region över längre tidsperioder, vanligtvis 30 år eller mer.',
    category: 'geography',
    difficulty: 'beginner',
  },
  {
    id: 'geo5',
    question: 'Vad är en tektonisk platta?',
    answer: 'En tektonisk platta är ett stort stycke av jordens litosfär (jordskorpa och övre mantel) som flyter på asthenosfären. Platttektonik förklarar jordbävningar, vulkanutbrott och bildandet av bergskedjor.',
    category: 'geography',
    difficulty: 'beginner',
  },
  
  // Add more flashcards for the other programs and categories as needed
];
...
export const LocalStorageProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    setFlashcards(storedFlashcards ? JSON.parse(storedFlashcards) : initialFlashcards);
    ...
    setUserStats(storedUserStats ? JSON.parse(storedUserStats) : initialUserStats);
  }, []);
  ...
  }, [flashcards, programs, categories, userStats]);

  // Add a new flashcard
  const addFlashcard = (flashcard: Omit<Flashcard, 'id'>) => {
    const newFlashcard: Flashcard = {
      ...flashcard,
      id: `${flashcard.category}-${Date.now()}`,
    ...
    toast({
      ...
      description: "Ditt nya flashcard har lagts till i samlingen.",
    });
  };
  ...
  const deleteFlashcard = (id: string) => {
    ...
    toast({
      ...
      description: "Flashcard har tagits bort från din samling.",
    });
  };
  ...
  const updateUserStats = (updates: Partial<UserStats>) => {
    setUserStats(prev => {
      ...
      if (!isToday) {
        if (isYesterday) {
          // Maintain streak if last activity was yesterday
          streak += 1;
          ...
          if (streak === 3) {
            addAchievement({
              ...
              description: 'Använt Learny.se 3 dagar i rad',
              icon: 'flame'
            });
          } else if (streak === 7) {
            addAchievement({
              name: '7-dagars Streak',
              description: 'Använt Learny.se en hel vecka i rad',
              icon: 'flame'
            });
          } else if (streak === 30) {
            addAchievement({
              name: '30-dagars Streak',
              description: 'Använt Learny.se en hel månad i rad!',
              icon: 'flame'
            });
          }
        } else {
  ...
  const addAchievement = (achievement: Omit<UserAchievement, 'id' | 'dateEarned'>) => {
    ...
    const hasAchievement = userStats.achievements.some(a => a.name === achievement.name);
  ...
  const markProgramCompleted = (programId: string) => {
    ...
    if (userStats.completedPrograms.includes(programId)) return;
    ...
    if (program) {
      addAchievement({
        ...
        description: `Du har slutfört programmet ${program.name}!`,
        icon: 'trophy'
      });
      
      // Check if all programs in a category are completed
      const categoryPrograms = programs.filter(p => p.category === program.category);
      const completedCategoryPrograms = categoryPrograms.filter(p => 
        userStats.completedPrograms.includes(p.id) || p.id === programId
      );
      
      if (categoryPrograms.length === completedCategoryPrograms.length) {
        const category = categories.find(c => c.id === program.category);
        if (category) {
          addAchievement({
            name: `${category.name} Mästare`,
            description: `Du har klarat alla program inom ${category.name.toLowerCase()}!`,
            icon: 'award'
          });
        }
      }
